#!/bin/bash
#SBATCH --job-name=originator_TBannotation_simulatedData_b_123
#SBATCH --nodes=1
...
/sw/pkgs/arc/stacks/gcc/10.3.0/R/4.3.1/bin/Rscript src/Main.R \
  ...
  --output '/nfs/dcmb-lgarmire/thatchau/originator_GB_revision/results/simulatedData_seeds/b/123' \
  --query-celltype-col 'final_annotation' \
  --ref-celltype-col 'level1_annotation' \
  --query-celltypes 'B-cell' \
