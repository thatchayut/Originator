#!/bin/bash
#SBATCH --job-name=originator_TBannotation_simulatedData_tcell_123
#SBATCH --nodes=1
...
/sw/pkgs/arc/stacks/gcc/10.3.0/R/4.3.1/bin/Rscript src/Main.R \
  --name 'TBannotation_simulatedData_tcell_123' \
  ...
  --output '/nfs/dcmb-lgarmire/thatchau/originator_GB_revision/results/simulatedData_seeds/tcell/123' \
  --query-celltype-col 'final_annotation' \
  --ref-celltype-col 'level1_annotation' \
  --query-celltypes 'T-cell' \
  --ref-celltypes 'T-cell' \
  --unified-celltypes 'T-cell' \
  --seed '123'
