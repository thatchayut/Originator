#!/bin/bash
#SBATCH --job-name=originator_${jobName}
#SBATCH --nodes=1
...
/sw/pkgs/arc/stacks/gcc/10.3.0/R/4.3.1/bin/Rscript copy_of_Yiwen_Originator_src_121624/Main.R \
  --name '${jobName}' \
  --path '${path}' \
  --reference '${reference}' \
  --output '${output}' \
  --query-celltype-col '${queryCelltypeCol}' \
  --ref-celltype-col '${refCelltypeCol}' \
  --query-celltypes '${queryCelltypes}' \
  --ref-celltypes '${refCelltypes}' \
  --unified-celltypes '${unifiedCelltypes}' \
  --seed '${seed}'
