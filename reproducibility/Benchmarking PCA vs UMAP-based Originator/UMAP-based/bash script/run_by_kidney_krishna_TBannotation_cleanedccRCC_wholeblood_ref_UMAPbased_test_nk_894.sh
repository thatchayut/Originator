#!/bin/bash
#SBATCH --job-name=originator_kidney_krishna_TBannotation_cleanedccRCC_wholeblood_ref_UMAPbased_test_nk_894
#SBATCH --nodes=1
...
/sw/pkgs/arc/stacks/gcc/10.3.0/R/4.3.1/bin/Rscript /nfs/dcmb-lgarmire/thatchau/originator_GB_revision/src/Main.R \
  --name 'kidney_krishna_TBannotation_cleanedccRCC_wholeblood_ref_UMAPbased_test_nk_894' \
  ...
  --output '/nfs/dcmb-lgarmire/thatchau/originator_GB_revision/results/kidney_krishna_seeds/TBannotation_cleanedccRCC_wholeblood_ref_UMAPbased_test/nk/894' \
  --query-celltype-col 'cell_type' \
  --ref-celltype-col 'level1_annotation' \
  --query-celltypes 'NK_cell' \
  --ref-celltypes 'nk cell' \
  --unified-celltypes 'NK-cell' \
  --seed '894'
