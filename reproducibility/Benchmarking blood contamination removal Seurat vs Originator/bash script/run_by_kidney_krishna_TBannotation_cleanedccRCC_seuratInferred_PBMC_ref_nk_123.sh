#!/bin/bash
#SBATCH --job-name=originator_kidney_krishna_TBannotation_cleanedccRCC_seuratInferred_PBMC_ref_nk_123
#SBATCH --nodes=1
...
/sw/pkgs/arc/stacks/gcc/10.3.0/R/4.3.1/bin/Rscript src/Main.R \
  --name 'kidney_krishna_withPairedPBMC_cleaned_nk' \
  ...
  --output '/nfs/dcmb-lgarmire/thatchau/originator_GB_revision/results/kidney_krishna_seeds/TBannotation_cleanedccRCC_seuratInferred_PBMC_ref/nk/123' \
  --query-celltype-col 'cell_type_consolidate' \
  --ref-celltype-col 'level1_annotation' \
  --query-celltypes 'NK cell' \
